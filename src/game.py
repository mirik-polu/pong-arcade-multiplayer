...
class PongGame(arcade.Window):
    def __init__(self):
        ...
        self.paddle_list.append(self.paddle1)
        self.paddle_list.append(self.paddle2)
    ...
    def apply_input(self):
        # Временная локальная логика. Позже заменим на self.network_inputs
    ...
    def update_physics(self):
        ...
        if collisions:
            self.ball.change_x *= -1.05  # небольшое ускорение
            # "Выталкивание" мяча, чтобы не застревал
    ...
    def on_draw(self):
        self.clear()
        arcade.draw_rectangle_filled(SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2, 4, SCREEN_HEIGHT, arcade.color.GRAY)
        arcade.draw_text(f"{self.score[0]} : {self.score[1]}", SCREEN_WIDTH / 2, SCREEN_HEIGHT - 50,
